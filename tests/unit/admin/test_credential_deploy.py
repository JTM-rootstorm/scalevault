...
def test_api_uses_bounded_systemd_pepper_credential_without_secret_environment() -> None:
    ...
    assert "LoadCredential=client-token-pepper:/etc/kivra-memory/client-token-pepper" in drop_in
    assert (
        "KIVRA_MEMORY_CLIENT_TOKEN_PEPPER_CREDENTIAL="
        "/run/credentials/kivra-memory-api.service/client-token-pepper" in drop_in
    )
...
def test_operator_documentation_uses_protected_files_and_explicit_secret_output() -> None:
    documentation = REPOSITORY_ROOT.joinpath("deploy/memory-node/systemd/README.md").read_text()
...
def test_console_entry_is_registered() -> None:
    ...
    assert 'kivra-memory-credential-admin = "kivra_memory.admin.credentials_main:main"' in project
