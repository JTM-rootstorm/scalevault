...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            ...
            "IF (NEW.last_verified_commit_id IS DISTINCT FROM OLD.last_verified_commit_id) "
            "IS DISTINCT FROM "
            "(NEW.last_verified_tree_id IS DISTINCT FROM OLD.last_verified_tree_id) THEN "
            "RAISE EXCEPTION 'ingress provider commit and tree must advance together' "
